...
- Single-point 세 모드 비교에서는 `gap, d = 3.5 Å`가 최안정으로 나왔지만, relaxation을 풀면 결과가 틀어질 수 있다. relaxed 구조를 본 다음에 판단한다.
- 최종 목표는 heterostructure transport I-V 커브이다. relaxation이 수렴하면 곧바로 transport 계산으로 연결한다.
- Transport 셀에서 `VSe3`가 얼마나 길어야 하는지가 중요한 변수이다. 1D 시스템은 screening이 약하므로 분자 주변 perturbation이 긴 range까지 살아있을 가능성이 있다.
- **Perturbation range를 실제로 측정하는 방법**: `분자 + VSe3`와 `VSe3-only` 두 구조의 SCF potential을 각각 계산하고 차이를 플롯한다. 그 차이가 충분히 감쇄하는 길이가 scattering region 최소 길이이며, 그 값으로 electrode/device 길이를 정한다.
- 셀이 커지면 HPC (Stampede3, Nurion) 자원을 제약 없이 쓰는 것으로 가정하고, 물리적으로 필요한 만큼 크게 잡는다.
- 계산 결과를 판단할 때는 수치 자체보다 `물리적으로 말이 되는가`를 먼저 본다. AI agent 출력도 독립적인 context로 재검증해서 의도 반영으로 생긴 편향을 걸러낸다.

### Decisions

- `gap 모드, d = 3.5 Å`를 relaxation 초기 구조로 확정한다.
- Transport 셀 길이는 relaxation 수렴 후 `potential perturbation range` 계산으로 결정한다.
- Relaxation이 수렴되면 구조를 실험팀에 공유한다.

### Action Items

- [ ] `gap, d = 3.5 Å` relaxation 수렴 확인
- [ ] `VSe3 + 분자` vs `VSe3-only` SCF 2회 → `ΔV(z)` 플롯 → perturbation range 추정
